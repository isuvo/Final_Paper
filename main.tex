...
Each objective is based on interprocedural, chain-focused causal reasoning that explicitly models how vulnerabilities emerge, propagate, and compound across procedure boundaries.
\medskip
...
\medskip
\noindent\textbf{O2 - Vulnerability causal chain construction:}  
...
This thesis makes the following contributions to interprocedural, causality-aware software vulnerability analysis.
...
A context-driven causal pipeline that constructs explicit interprocedural root-to-sink vulnerability chains. This approach constructs the full propagation path (source→propagation→sink) as the main explanatory unit.

\medskip
\noindent\textbf{C3 – Causal evaluation criteria for vulnerability detection:}  
This thesis evaluates causal vulnerability chains using both conventional and causal metrics to test whether model decisions are consistent and grounded in causal evidence.
...
This thesis is organized as follows. Chapter~1 introduces the problem, research gap, goals, and main contributions. Chapter~2 reviews related work in program analysis, data-driven vulnerability detection, and causal/explainable methods. Chapter~3 presents the methodology, including dataset preparation, graph construction, and the learning pipeline. Chapter~4 describes the evaluation protocol and metrics, and Chapter~5 reports experimental results, robustness studies, and qualitative case examples. Chapter~6 concludes with a summary of findings and directions for future work. The remaining chapters form the appendix, collecting core algorithms, formal model equations, detailed metric definitions, extended results and hyperparameters, the dataset card and role lexicon, and artifact and reproducibility details.
...
\smallskip
\noindent \textbf{Dynamic analysis} executes the program (or symbolic abstractions) to observe concrete behaviors. Fuzzing mutates inputs to trigger failures; sanitizers instrument code to detect memory errors and undefined behavior at runtime; concolic/symbolic execution uses path constraints to steer toward hard-to-reach states. Dynamic analysis yields concrete \emph{witness traces} but faces input-space and path-explosion limits. In practice, static analysis can prioritize likely vulnerable chains, while targeted dynamic validation confirms executability. This complementarity is especially useful for interprocedural mechanisms that cross module boundaries and depend on calling contexts.

\subsection{Code Structures for Security Analysis}
\label{sec:intro-repr}

\medskip
\noindent \textbf{Abstract Syntax Tree (AST):}
...
\medskip
\noindent \textbf{Data-Flow Graph (DFG):}
...
\medskip
\noindent \textbf{Program Dependence Graph (PDG):}
...
\medskip
\noindent \textbf{Code Property Graph (CPG):}
...
\medskip
\noindent \textbf{Interprocedural Semantics:}
...
\medskip
Auxiliary decoding diagnostics (e.g., Chain Success Rate, admissible expansion ratio, prior influence rate) are defined in Appendix~\ref{app:metrics-diag}.


\chapter{Experimental Results and Analysis}
\label{chap:results}

This chapter evaluates the chain-centric interprocedural vulnerability detection approach, emphasizing on causal validity, efficiency, and reproducibility. Results are organized to show how each component contributes to overall performance, making it straightforward to extend the system without breaking the methodological desig


\section{Experimental Setup and Configuration}
\label{sec:results-setup}

Experiments use the official \texttt{ReposVul} project-disjoint splits. Two encoders—\emph{Struct-only} and \emph{GCBERT+Struct} are compared, with the language model frozen. Decoding employs ACC-constrained beam search enhanced by a decoding-only Causal Knowledge Graph prior. 
...
\noindent\textbf{Model variants:} Model variants include Struct-only, using compact structural features (types, degrees, SSA hints, literals), and GCBERT+Struct, which adds frozen GraphCodeBERT embeddings. Both share identical graph topology.
...
\noindent Decoding uses ACC-constrained beam search, allowing steps with valid CFG reachability, consistent ARG→PARAM and RET→CALLER/LHS bindings, passed alias checks, and maintained stack discipline. A CKG prior mined from training chains adjusts move scores at inference but does not change admissibility.
...
For both encoder variants, this section reports standard graph-level metrics: Accuracy, Precision, Recall, F1, AUROC, and AUPRC. The operating threshold \(\tau_{\text{F1}^\star}\) is set on the validation split and applied unchanged to the test set. Probability calibration is done via temperature scaling on validation and reused on test. AUPRC is the main metric, with F1 and AUROC providing context due to the test set’s class imbalance (positives \(\approx 2.9\%\)). Results are averaged over five seeds.
...
\begin{table}[H]
	\centering
	\small
	\caption{Operating points and calibration (ECE, 15 bins; T from Valid).}
	...
	\begin{tabular}{l l c c c c}
		...
		\midrule
		Test  & Struct-only      & 0.32 & 1.41 & 0.082 & \textbf{0.036} \\
		Test  & GCBERT+Struct    & 0.27 & 1.29 & 0.064 & \textbf{0.022} \\
		\bottomrule
	\end{tabular}
\end{table}


\noindent Calibration improves Expected Calibration Error (ECE) on both variants after scaling. Precision-Recall (PR) and ROC curves reflect the same ordering.
...
False positives occur when safe code is incorrectly flagged as vulnerable. False negatives arise when true vulnerabilities are missed, frequently in macro-expanded code or callback-driven execution paths.

\section{Executable Chain Quality and Interprocedural Evidence}
\label{sec:results-chain-quality}

Chains are decoded as executable sequences using ACC-constrained beam search with \(K{=}8\), \(B{=}24\), \(H{=}5\), and \(\alpha{=}0.7\). A decoding-only CKG prior with \(\lambda{=}0.2\) reweights admissible moves. Statistics are computed on positive slices and averaged over five seeds. A chain is counted as feasible only if interprocedural control flow is realizable, data flow or guard preservation holds, call/return pairs are matched, and alias checks pass. Under these criteria, the GCBERT+Struct encoder improves feasibility by roughly 8–9 points on both validation and test splits.
...
\noindent  The IPA rate, the fraction of chains using both call and return edges, and the mean call depth are all reported for cross-boundary edges (CALL, ARG→PARAM, and RET→CALLER/LHS).
...
\noindent
Node and edge coverage, role coverage (source, sanitizer, propagator, sink), and order agreement (LCS) are measured. Sanitizer recovery and edge coverage show the largest gains, and order agreement also improves.
...
\noindent
 Hop count, number of files crossed, and percentage of summary edges are measured. Chains remain short and typically span only a few files. Reliance on summary edges decreases while agreement improves.
...
In summary, the \emph{GCBERT+Struct} variant is more feasible, has better interprocedural use, better role and edge coverage, a higher LCS, and a chain length that is about the same. 

\section{Causal Faithfulness}
\label{sec:res-causal}

I assess prediction dependence on reconstructed chains through minor and major counterfactual edits. The \textit{Counterfactual Consistency Score} (CCS) measures probability change magnitude. The \textit{Directional Consistency Rate} (DCR) shows the share of edits causing sign flips. The \emph{Causal Feature Attribution Measure} (CFAM) quantifies how much attribution mass lies on the predicted chain, while the chain invalidation rate counts cases where no valid chain remains after editing.
...
\noindent Small CCS appears after minor changes. When the sink is neutralized, a larger CCS shows up supports the hypothesis that the chain endpoint drives the prediction.
...
\noindent Direction mostly aligns with the intervention, and the GCBERT+Struct variant improves these scores by about 0.03–0.04 on both splits.
...
\noindent Table~\ref{tab:cf-cfam-role} shows CFAM scores overall and by chain segment (source, sanitizer, propagator, sink) on a [0,1] scale. Attribution matches the returned chain. The enriched variant increases on-chain share, especially for propagators and sanitizers.
...
\noindent Editing the sink removes nearly all admissible chains. Minor edits invalidate about half. Surviving chains show large score drops after sink edits.
...
Evidence is consistent across splits: minor changes yield small CCS and high DCR, while major changes cause large CCS, higher DCR, and near-total chain invalidation. CFAM rises with the enriched encoder, focusing attribution on key segments. These results confirm that predictions rely on executable chains, not random context.


\section{Efficiency and Decoding Dynamics}
\label{sec:res-efficiency}

Table~\ref{tab:eff-latency} shows inference time per graph. Most of the time is spent in the encoder forward pass, while ACC checks and the CKG prior add only a small extra delay.
...
Structural and ACC checks remove most candidate moves during decoding. CFG reachability alone filters out about 54--63\% of expansions (Appendix~\ref{app:beam-acc}, Table~\ref{tab:eff-beam-diagnostics}). Increasing the beam from \((4,12,3)\) to \((8,24,5)\) raises Validity by 4.2 points, IPA by 13.1 points, and LCS by 6.3 points, at an extra 6.9\,ms per graph. Since most latency stems from the encoder, the default beam offers a good balance of chain quality and runtime.
...
This thesis uses the C/C++ subset with the official project-level splits. Each entry contains a code snapshot, with aligned views at line, function, file, and repository level, and repository-scope caller–callee links.
...
Official train/validation/test splits are used without modification. Projects do not cross partitions, parent/child patches are never split, and identical files/CVEs are removed across splits to reduce leakage.
