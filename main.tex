...
This chapter explains how the method is evaluated. Two aspects are considered: (i) conventional classification quality, and (ii) semantics-grounded behavior, that is, whether predicted evidence forms executable interprocedural chains consistent with program semantics. Only basic equations are kept here for readability. Extended definitions and hyperparameters are provided in Appendix~\ref{app:metrics} and Appendix~\ref{app:extended}.
...
\noindent\textbf{Inputs:} Models operate on chain-prepared interprocedural CPGs (AST, CFG, DFG) enriched with \texttt{CALL}, \texttt{ARG}$\!\to$\texttt{PARAM}, and \texttt{RET}$\!\to$\texttt{CALLER}/\texttt{RET}$\!\to$\texttt{LHS} relations and alias summaries (Section~\ref{sec:method-repr}).
...
\noindent\textbf{Decoding:} Inference uses beam search constrained by ACC, with a decoding-only CKG prior mined from training data. The score combination is given in App.~\ref{app:eq-ckg}. Numeric settings and software versions appear in App.~\ref{app:extended-hparams}.
...
\noindent\textbf{Validity:} The fraction of returned chains that satisfy ACC feasibility checks is reported. These checks include CFG reachability, def–use or guard consistency, interprocedural call–return discipline, and alias coherence. The formal definition is in App.~\ref{app:eq-validity}.

\noindent\textbf{Structural fidelity:} When a reference chain is available, the following are reported: node and edge coverage, the longest common subsequence ratio, and role-aware coverage for \{\textit{source, sanitizer, propagator, sink}\}. A scalar Chain Overlap (CO) aggregates node/edge recovery. Formulas are in App.~\ref{app:eq-struct}.

\noindent\textbf{Interprocedurality:} The IPA Rate quantifies the share of predicted chains that traverse \texttt{CALL}/\texttt{ARG}$\!\to$\texttt{PARAM}/\texttt{RET} edges when such edges exist in the slice (App.~\ref{app:eq-ipa}).

\section{Counterfactual Metrics}
\label{sec:eval-cf}

Three specific changes are made to the system to see how it reacts to targeted code changes: guard strengthening, call unbinding, and sink neutralization.  Results are presented individually for each intervention type, facilitating a comprehensive analysis of the system's response.  App. \ref{app:eq-ccs} and App. \ref{app:eq-cfam} give more information about confidence intervals and different methods.  The CKG prior stays on during all of these evaluations, except in the robustness analysis column, where it is turned off so that performance can be measured on the new graph.
...
\end{equation}
For the graph  Before the edit, $p_i$ is the predicted probability, and after the edit, $p_i^{\mathrm{do}}$ is the new probability (both are between 0 and 1).  A small value means not much has changed, while a large value means a big change has happened.  In App. \ref{app:eq-ccs}, we define a directional consistency rate that checks to see if the change is going in the right direction for each type of edit.
...
\end{equation}
$F_c$ and $F_s$ are sets of features that are both on-chain and off-chain.  The attribution given to feature $f$ for graph $i$ is $A_i(f)\!\ge\!0$.  The numerator adds up the attribution for features that are on the chain that was returned.  The denominator adds up the attribution for all the features.  Values that are closer to $1$ mean that the features on the returned chain support the choices.  You can find normalization choices in App. \ref{app:eq-cfam}.  The post-edit chain invalidation rate is also shown as a quick check.

\section{Decoding Diagnostics}
\label{sec:eval-diag}

There are four indicators that sum up decoding dynamics: the Chain Success Rate, the average chain length, the interprocedural hop ratio, and the ACC rejection mix.  App. \ref{app:metrics-diag} and the supplement show more diagnostics, such as the admissible expansion ratio, motif coverage, and prior influence rate.

\section{Reproducibility and Artifact}
\label{sec:eval-artifact}

The dataset has predictions for each graph, chains with beam traces, calibration parameters, intervention logs, configuration files, seeds, environment hashes, the CKG prior, and decoding diagnostics. Reproducibility scripts make it easier to regenerate graphs from raw repositories and recalculate all of the tables and figures. App gives you a full list of everything and step-by-step instructions. ~\ref{app:artifact}.
...
We report definitions for completeness.
\begin{align}
...
If you start from raw sources (ReposVul file lists), run the preprocessing pipeline; if you already have prepared shards, you can skip to Step 3.
\textbf{Linux}
\begin{verbatim}
...
Choose the trainer you use for the paper (both call the same backbone).
\textbf{Linux}
\begin{verbatim}
...
If your environment and paths match the defaults, this condensed sequence runs the full pipeline.
